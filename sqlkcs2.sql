...
-- ------------------------------------------------------
SELECT * FROM pedidos LIMIT 30;
-- ------------------------------------------------------
SELECT pedido_numero, COUNT(*) AS linhas_por_pedido
FROM pedidos
GROUP BY pedido_numero
ORDER BY linhas_por_pedido DESC
LIMIT 10;
-- ------------------------------------------------------
CREATE DATABASE IF NOT EXISTS sistema_vendas_normalizado;
USE sistema_vendas_normalizado;
-- ------------------------------------------------------
...
-- ------------------------------------------------------
SELECT cliente_id,
       GROUP_CONCAT(DISTINCT cliente_nome SEPARATOR ' | ') AS nomes,
       GROUP_CONCAT(DISTINCT cliente_email SEPARATOR ' | ') AS emails,
       COUNT(*) AS ocorrencias
FROM sistema_vendas.pedidos
GROUP BY cliente_id
HAVING COUNT(DISTINCT cliente_nome) > 1
   OR COUNT(DISTINCT cliente_email) > 1;
-- ------------------------------------------------------
SELECT produto_id,
       GROUP_CONCAT(DISTINCT produto_descricao SEPARATOR ' | ') AS descricoes,
       GROUP_CONCAT(DISTINCT preco_unitario SEPARATOR ' | ') AS precos
FROM sistema_vendas.pedidos
GROUP BY produto_id
HAVING COUNT(DISTINCT produto_descricao) > 1
   OR COUNT(DISTINCT preco_unitario) > 1;

-- ------------------------------------------------------
INSERT INTO clientes (cliente_id, nome, endereco, telefone, email)
SELECT DISTINCT cliente_id, cliente_nome, cliente_endereco, cliente_telefone, cliente_email
FROM sistema_vendas.pedidos;
-- ------------------------------------------------------
INSERT INTO produtos (produto_id, descricao, categoria, preco_unitario)
SELECT DISTINCT produto_id, produto_descricao, produto_categoria, preco_unitario
FROM sistema_vendas.pedidos;

-- ------------------------------------------------------
INSERT INTO pedidos (pedido_id, data_pedido, valor_total, forma_pagamento, cliente_id)
SELECT DISTINCT pedido_numero, data_pedido, valor_total_pedido, forma_pagamento, cliente_id
FROM sistema_vendas.pedidos;
-- ------------------------------------------------------
INSERT INTO itens_pedido (pedido_id, produto_id, quantidade, valor_item)
SELECT pedido_numero, produto_id, quantidade, (preco_unitario * quantidade)
FROM sistema_vendas.pedidos;
-- ------------------------------------------------------
UPDATE pedidos p
JOIN (
    SELECT pedido_id, SUM(valor_item) AS total_calc
    FROM itens_pedido
    GROUP BY pedido_id
) t ON p.pedido_id = t.pedido_id
SET p.valor_total = t.total_calc;
...
WHERE pr.produto_id IS NULL;

-- ------------------------------------------------------
SELECT p.pedido_id, p.valor_total, SUM(i.valor_item) AS soma_itens
FROM pedidos p
JOIN itens_pedido i ON p.pedido_id = i.pedido_id
GROUP BY p.pedido_id
HAVING ABS(p.valor_total - SUM(i.valor_item)) > 0.01;

-- ------------------------------------------------------
ALTER TABLE pedidos ADD INDEX idx_cliente (cliente_id);
ALTER TABLE itens_pedido ADD INDEX idx_produto (produto_id);
ALTER TABLE itens_pedido ADD INDEX idx_pedido (pedido_id);
